...
# #### 4.1.1. - Esquema de modulación QPSK
...
def modulador_I(bits, fc, mpp):
    ...
    bits_r = bits.reshape(len(bits)//2,2)
    
    # 1. Parámetros de la 'señal' de información (bits)
    N = len(bits_r) # Cantidad de bits
...
def modulador_Q(bits, fc, mpp):
    ...
    return senal_Q, portadora_I, moduladora_Q 
    
# Suma de las señales en I y en Q
# Función señal modulada
def senal_modulada(senal_I, senal_Q, moduladora_I, moduladora_Q, t_simulacion, Tc, N):
    
    # Suma de las portadoras
    senal_Tx = senal_I + senal_Q
    
    #Suma de las moduladoras
...
# #### 4.1.2. - Construcción de un canal con ruido AWGN
...
def demodulador(senal_Rx, portadora_I, portadora_Q, mpp):
    ...
    # Energía de un período de la portadora I
    Es_I = np.sum(portadora_I**2)

    # Energía de un período de la portadora Q
...
# #### 4.1.4. - Reconstrucción de la imagen
...
# #### 4.1.5. - Simulación del sistema de comunicaciones con modulación QPSK
...
# 7. Cálculo del tiempo de simulación
print('Duración de la simulación: ', time.time() - inicio)
